...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create ngram-dashboard-0c2f
  fly apps create ngram-dashboard-0c2f-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app ngram-dashboard-0c2f
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app ngram-dashboard-0c2f-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'ngram-dashboard-0c2f-staging' does not match app name in config file 'ngram-dashboard-0c2f'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name ngram-dashboard-0c2f-db
  fly postgres attach --app ngram-dashboard-0c2f ngram-dashboard-0c2f-db

  fly postgres create --name ngram-dashboard-0c2f-staging-db
  fly postgres attach --app ngram-dashboard-0c2f-staging ngram-dashboard-0c2f-staging-db
  ```
